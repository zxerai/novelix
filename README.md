...
jiaos write next 吞天魔帝 --count 10
```

打开 Studio Web 工作台（`jiaos` → `http://localhost:4567`），在浏览器里管理书籍、审阅章节、看数据分析。

> **🎯 3 分钟上手**：安装 → `jiaos init` → `jiaos doctor` 配模型 → 写书
...
### 🎭 文风仿写
`jiaos style analyze` 提取参考文本的统计指纹（句长分布、词频、节奏），`jiaos style import` 注入书籍，后续章节自动采用该风格。

### 📝 创作简报
`jiaos book create --brief my-ideas.md` 传入脑洞和设定，Architect 基于简报生成设定，而非凭空创作。

### 🔄 续写 + 同人
`jiaos import chapters` 导入已有小说，自动逆向 7 个真相文件，无缝续写。`jiaos fanfic init` 从原作创建同人书（canon/au/ooc/cp 四种模式）。
...
```
或写 `.env`：`JIAOS_LLM_BASE_URL` + `JIAOS_LLM_API_KEY` + `JIAOS_LLM_MODEL`。

### 多模型路由（可选）
```bash
jiaos config set-model writer <model> --provider <provider>
jiaos config show-models     # 查看当前路由
```

### 诊断
```bash
jiaos doctor      # 检查配置、API 连通性
```
...
```bash
jiaos plan chapter 吞天魔帝 --context "本章先把注意力拉回师徒矛盾"
jiaos compose chapter 吞天魔帝
```
...
### 1. 完整管线（一键式）
```bash
jiaos write next 吞天魔帝           # 草稿 → 审计 → 自动修订
jiaos write next 吞天魔帝 --count 5 # 连续写 5 章
```

### 2. 原子命令（可组合）
```bash
jiaos plan chapter 吞天魔帝 --context "师徒矛盾" --json
jiaos compose chapter 吞天魔帝 --json
jiaos draft 吞天魔帝 --json
jiaos audit 吞天魔帝 31 --json
jiaos revise 吞天魔帝 31 --json
```

### 3. 自然语言 Agent 模式
```bash
jiaos agent "帮我写一本都市修仙，主角是个程序员"
jiaos agent "写下一章，重点写师徒矛盾"
```
...
| Command | Description |
|---------|-------------|
| `jiaos init [name]` | 初始化项目 |
| `jiaos book create` | 创建新书（`--genre`, `--brief <file>` 创作简报） |
| `jiaos book list` | 列出书籍 |
| `jiaos book delete <id>` | 删除书籍 |
| `jiaos write next [id]` | 完整管线写下一章（`--count`, `--words`, `-q`） |
| `jiaos write rewrite [id] <n>` | 重写第 N 章 |
| `jiaos draft [id]` | 只写草稿 |
| `jiaos audit [id] [n]` | 审计 |
| `jiaos revise [id] [n]` | 修订（`--mode anti-detect` 反检测） |
| `jiaos agent <instruction>` | 自然语言模式 |
| `jiaos review list/approve-all [id]` | 审阅草稿 |
| `jiaos status [id]` | 项目状态 |
| `jiaos export [id]` | 导出（`--format txt/md/epub`） |
| `jiaos short run` | 写短篇 |
| `jiaos fanfic init` | 创建同人书 |
| `jiaos config set-model <agent> <model>` | 多模型路由 |
| `jiaos doctor` | 诊断配置 |
| `jiaos detect [id] [n]` | AIGC 检测 |
| `jiaos style analyze/import` | 文风分析/导入 |
| `jiaos import chapters [id]` | 导入续写 |
| `jiaos studio` / `jiaos` | Web 工作台 |
| `jiaos up / down` | 守护进程 |
